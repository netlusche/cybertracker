...
const PromptModal = ({ message, onConfirm, onCancel }) => {
    ...
    return (
        ...
        >
            ...
            <div className="flex justify-end gap-2">
                <CyberButton variant="ghost" onClick={onCancel}>{t('common.cancel')}</CyberButton>
                <CyberButton variant="primary" onClick={() => onConfirm(value)}>{t('common.submit')}</CyberButton>
            </div>
        </SystemModal>
    );
};
...
const AdminPanel = ({ onClose }) => {
    ...
    const fetchSettings = async () => {
        try {
            const res = await apiFetch('api/index.php?route=admin/settings');
            if (res.ok) {
    ...
    const fetchUsers = async (page = 1, sortKey = sortConfig.key, sortDir = sortConfig.direction, search = debouncedSearch) => {
        try {
            const res = await apiFetch(`api/index.php?route=admin/users&page=${page}&sort=${sortKey}&dir=${sortDir}&search=${encodeURIComponent(search)}`);
            if (res.ok) {
                const data = await res.json();
                setUsers(data.users);
                setPagination({
    ...
    const handleSort = (key) => {
        ...
        fetchUsers(pagination.currentPage, key, direction, debouncedSearch);
    };
    ...
    const performToggleRole = async (user, newRole) => {
        try {
            const res = await apiFetch('api/index.php?route=admin/users/role', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    const performDeleteUser = async (id, username) => {
        try {
            const res = await apiFetch('api/index.php?route=admin/users/delete', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    const handleToggleVerified = async (user) => {
        try {
            const res = await apiFetch('api/index.php?route=admin/users/toggle_verified', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    const performResetPassword = async (id, username, newPass) => {
        if (!newPass) return;
        try {
            const res = await apiFetch('api/index.php?route=admin/users/reset_password', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    const performDisable2FA = async (id, username) => {
        try {
            const res = await apiFetch('api/index.php?route=admin/users/disable_2fa', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ target_id: id })
            });
    ...
    const columns = [
        {
            ...
            label: 'ID',
            sortable: true,
            cellClassName: "text-gray-400 font-bold",
            render: (u) => `#${u.id}`
        },
        {
            key: 'username',
            label: t('admin.col_codename'),
            sortable: true,
            cellClassName: "text-white font-bold"
        },
        {
            key: 'role',
            ...
            render: (u) => (
                <>
                    <span className={`text-xs px-2 py-1 rounded border ${u.role === 'admin' ? 'border-cyber-primary text-cyber-primary' : 'border-gray-500 text-gray-300'}`}>
                        {u.role.toUpperCase()}
                    </span>
    ...
    return (
        <SystemModal
            ...
            variant="primary"
            maxWidth="max-w-4xl"
            closeBtnClassName={theme === 'lcars' ? 'bg-[#ffaa00] text-black px-4 py-1 rounded-tr-[1.5rem] hover:brightness-110 text-xl' : ''}
        >
            {/* Modals */}
            {confirmAction && (
                <ConfirmModal
            ...
            <div className="flex justify-end mb-4 relative z-10 w-full mt-2">
                <input
                    ...
                    onFocus={(e) => e.target.select()}
                    className="bg-black/50 border border-cyber-primary/50 text-cyber-primary px-3 py-1 pr-8 text-sm focus:outline-none focus:border-cyber-primary w-64 rounded-sm tracking-widest placeholder-cyber-primary/50 input-normal-case"
                />
            ...
            <DataGrid
                ...
                onSort={handleSort}
                keyExtractor={(u) => u.id}
                emptyMessage={t('tasks.no_directives')}
            />

            {/* Pagination Controls */}
            <div className="flex items-center justify-between border-t border-cyber-primary/30 pt-4 mt-6 text-xs font-mono">
                <div className="text-gray-400">
                    {t('admin.total_recruits')}: <span className="text-cyber-primary">{pagination.totalUsers}</span>
                </div>
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-cyber-primary px-2">
